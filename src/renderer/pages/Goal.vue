    ...
    <el-form ref="goal" :model="goal" :rules="rules">
      ...
      <el-col :span="5">
        <el-form-item label="预计日期" prop="goaldate">
          <el-date-picker v-model="goal.goaldate" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" placeholder="选择日期"
            style="width:135px">
          </el-date-picker>
        </el-form-item>
      </el-col>
      <el-col :span="5">
        <el-form-item label="预期金额" prop="amount">
          <el-input v-model="goal.amount" placeholder="请输入金额" style="width:135px"></el-input>
        </el-form-item>
      </el-col>
      <el-col :span="16">
  ...
  methods: {
    onConfirm (goal) {
      this.$refs[goal].validate((valid) => {
        if (valid) {
          db.serialize(() => {
            db.run(`INSERT INTO schemes_project_info(type,content,finaldate,amount,status) values (?,?,?,?,?)`, ['goal', this.goal.objective, this.goal.goaldate, this.goal.amount, 'continuing'])
            db.get(`select * from schemes_project_info WHERE type = 'goal' order by id desc limit 1`, [], (err, row) => {
              if (err) {
                throw err
              }
    ...
    getTableData (pageSize, page) {
      db.all(`select *,strftime('%Y-%m-%d',created) as createdf,strftime('%Y-%m-%d',finaldate) as finaldatef,strftime('%Y-%m-%d',finished) as finishedf from schemes_project_info WHERE type = 'goal' order by status limit ? offset ?`, [pageSize, page], (err, rows) => {
        if (err) {
          throw err
        }
    ...
    updateGoal () {
      if (this.rowStatus && this.rowStatus !== 'continuing') {
        ...
        this.goalTableData[this.rowIndex].finishedf = this.finshedDate.substring(0, 10)
      }
