    ...
    <el-form ref="schedule" :model="schedule" :rules="rules">
      ...
      <el-col v-show="customDays" :span="5">
        <el-form-item prop="customDyas" label="天数">
          <el-input
            placeholder="请输入天数"
            v-model.number="schedule.customDays"
            style="width: 100px"
          >
          </el-input>
        </el-form-item>
    ...
    >
      ...
      <el-table-column label="最近完成" width="100">
        <template v-slot="scope">
          <el-button
            type="text"
            @click.stop="handleRound(scope.$index, scope.row)"
          >
            {{ scope.row.thisround }}
          </el-button>
        </template>
  ...
  methods: {
    ...
    handleInfo() {
      db.run(
        ...
        [this.tempRow.id, this.finshedDate, this.handleComment]
      );
